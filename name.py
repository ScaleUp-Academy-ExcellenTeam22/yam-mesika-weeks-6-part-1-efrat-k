...
"""
def full_names(first_name, lest_name,min_length=0):
    if min_length ==0:
        names=[(first,last) for first in first_name for last in lest_name]
    else:
        names = [(first, last) for first in first_name for last in lest_name if len(first)+len(last)+1>=min_length]
    full = []
    for i in names:
        a=i[0][0].upper() + i[0][1:]
        b=i[1][0].upper()+i[1][1:]
